...
public class OrdemDeCompraPagSeguroControllerDev {

    private final OrdemDeCompraService ordemDeCompraService;
    private final PagSeguroClient gatewayClient;

    @Autowired
    public OrdemDeCompraPagSeguroControllerDev(OrdemDeCompraServiceImpl ordemDeCompraService, PagSeguroClient gatewayClient){
    ...
    public ResponseEntity<?> cadastrarOrdemDeCompra(@RequestBody @Valid OrdemCompraRequest ordemCompraRequest) throws MalformedURLException {
        ...
        Compra compra = this.ordemDeCompraService.abrirOrdemDeCompra(ordemCompraRequest.toModel(UserLogadoMock.simularUsuarioLogado(), "PagSeguro"));
