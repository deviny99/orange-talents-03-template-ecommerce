...
@RestController
@RequestMapping("/paypal/transacao")
public class TransacaoPaypalController {

    private final TransacaoRepository transacaoRepository;

    @Autowired
    public TransacaoPaypalController(TransacaoRepository transacaoRepository){
    ...
    public ResponseEntity<?> aprovarTransacao(@PathVariable("id") Long idTransacao){
        ...
        if(transacaoOptional.isPresent()){
            ...
            body.put("status", String.valueOf(1));
    ...
    public ResponseEntity<?> negarTransacao(@PathVariable("id") Long idTransacao){
        ...
        if(transacaoOptional.isPresent()){
            ...
            Map<String,String> body = new HashMap<>();
            body.put("status", String.valueOf(0));
            body.put("url","http://localhost:8080/paypal/transacao/"+transacao.getIdCompra());
            this.transacaoRepository.save(transacao);
            notificarClient(transacao.getUrlNotification().getUrl(),
                    transacao.getUrlNotification().getMethod().getMethod(),
                    transacao.getIdCompra(),
